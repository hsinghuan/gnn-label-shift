...
def main(args):
    ...
    if args.model == "logreg":
        ...
        if args.method == "iw-erm" or args.method == "erm":
            adapter = IWERM(wt, model, device)
            adapter.adapt(data_src, args)
        elif args.method == "tbd":
            adapter = Tbd(wt, model, device)
            subsample_portion_list = [0.5]
            adapter.adapt(data_src, data_tgt, subsample_portion_list, args)
        ...
        # torch_fit(model, args.model, data_src, optimizer, args.model_epochs, class_weight=wt_tensor)
    ...
    if args.method == "iw-erm":
        exp_method = args.blackbox + "_" + args.estimator
    else:
        exp_method = args.method

    layer_config = str(len(args.model_gnn_dim_list) - 1) + "layer" if args.model not in ["logreg", "mlp"] else ""
    result_subdir = os.path.join(args.result_dir, args.exp_name, exp_method, args.model, layer_config, str(args.seed))
